...
def run_rabbitmqctl(*args)
  cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl #{args.join(' ')}"
...
def cluster_status
  ...
  cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl cluster_status"
  Chef::Log.debug("[rabbitmq_cluster] Executing #{cmd}")
  cmd = get_shellout(cmd)
  cmd.run_command
  cmd.error!
...
def node_name
  ...
  cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl eval \"node().\" | head -1"
  Chef::Log.debug("[rabbitmq_cluster] Executing #{cmd}")
  cmd = get_shellout(cmd)
  cmd.run_command
  cmd.error!
...
def join_cluster(cluster_name, type)
  cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl join_cluster #{type == 'ram' ? '--ram' : ''} #{cluster_name}".squeeze(' ')
  Chef::Log.debug("[rabbitmq_cluster] Executing #{cmd}")
  cmd = get_shellout(cmd)
  cmd.run_command
  begin
...
def change_cluster_node_type(cluster_node_type)
  cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl change_cluster_node_type #{cluster_node_type}"
  Chef::Log.debug("[rabbitmq_cluster] Executing #{cmd}")
  cmd = get_shellout(cmd)
  cmd.run_command
  begin
