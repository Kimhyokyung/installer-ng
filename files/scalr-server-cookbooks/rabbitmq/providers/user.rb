...
def user_exists?(name)
  cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl -q list_users |grep '^#{name}\\s'"
...
def user_has_tag?(name, tag)
  cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl -q list_users"
...
def user_has_permissions?(name, vhost, perm_list = nil)
  ...
  cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl -q list_user_permissions #{name} | grep \"^#{vhost}\\s\""
  cmd = Mixlib::ShellOut.new(cmd, :env => shell_environment)
  cmd.run_command
...
action :add do
  unless user_exists?(new_resource.user)
    ...
    cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl add_user #{new_resource.user} '#{new_password}'"
...
action :delete do
  if user_exists?(new_resource.user)
    cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl delete_user #{new_resource.user}"
    execute cmd do
      environment shell_environment
...
action :set_permissions do
  ...
  vhosts.each do |vhost|
    ...
    cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl set_permissions #{vhostopt} #{new_resource.user} \"#{perm_list.join('" "')}\""
    execute cmd do
      environment shell_environment
...
action :clear_permissions do
  ...
  vhosts.each do |vhost|
    ...
    vhostopt = "-p #{vhost}" unless vhost.nil?
    cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl clear_permissions #{vhostopt} #{new_resource.user}"
    execute cmd do
      environment shell_environment
...
action :set_tags do
  ...
  unless user_has_tag?(new_resource.user, new_resource.tag)
    cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl set_user_tags #{new_resource.user} #{new_resource.tag}"
    execute cmd do
      environment shell_environment
...
action :clear_tags do
  ...
  unless user_has_tag?(new_resource.user, '"\[\]"')
    cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl set_user_tags #{new_resource.user}"
    execute cmd do
      environment shell_environment
...
action :change_password do
  if user_exists?(new_resource.user)
    cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl change_password #{new_resource.user} #{new_resource.password}"
