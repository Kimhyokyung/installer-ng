...
def vhost_exists?(name)
  cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl -q list_vhosts | grep ^#{name}$"
...
action :add do
  unless vhost_exists?(new_resource.vhost)
    cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl add_vhost #{new_resource.vhost}"
    execute cmd do
...
action :delete do
  if vhost_exists?(new_resource.vhost)
    cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl delete_vhost #{new_resource.vhost}"
    execute cmd do
