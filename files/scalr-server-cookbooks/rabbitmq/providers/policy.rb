...
def policy_exists?(vhost, name)
  cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl list_policies"
...
action :set do
  cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl set_policy"
...
action :clear do
  if policy_exists?(new_resource.vhost, new_resource.policy)
    cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl clear_policy #{new_resource.policy}"
    cmd += " -p #{new_resource.vhost}" unless new_resource.vhost.nil?
...
action :list do
  execute 'list_policies' do
    command "HOME=#{node.rabbitmq.user_home} rabbitmqctl list_policies"
    environment shell_environment
  end
