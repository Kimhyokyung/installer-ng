...
def parameter_exists?(vhost, name)
  cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl list_parameters"
...
action :set do
  cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl set_parameter"
...
action :clear do
  if parameter_exists?(new_resource.vhost, new_resource.parameter)
    ...
    cmd = "HOME=#{node.rabbitmq.user_home} rabbitmqctl clear_parameter #{parameter}"
    cmd << " -p #{new_resource.vhost}" unless new_resource.vhost.nil?
...
action :list do
  execute 'list_parameters' do
    command "HOME=#{node.rabbitmq.user_home} rabbitmqctl list_parameters"
    environment shell_environment
  end
